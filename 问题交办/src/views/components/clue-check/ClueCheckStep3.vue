  ...
  <div class="page-container">
    ...
    <el-card shadow="always" class="main-card" v-loading="loading">
      ...
      >
        ...
        <el-step title="整改情况" description="线索核查 - 整改情况" />
      </el-steps>

      <!-- 表单内容 -->
      <div class="content-area">
        <h3 style="color: #217346; margin-bottom: 20px">线索核查 - 整改情况</h3>
        ...
        >
          <el-row :gutter="20">
            <el-col :span="12">
              <el-form-item label="核查开始时间" prop="checkStartTime">
                <el-date-picker
                  v-model="formData.checkStartTime"
                  type="datetime"
                  placeholder="请选择核查开始时间"
                  style="width: 100%"
                />
              </el-form-item>
            </el-col>
            ...
            <el-col :span="12">
              <el-form-item label="核查完成时间" prop="checkEndTime">
                <el-date-picker
                  v-model="formData.checkEndTime"
                  type="datetime"
                  placeholder="请选择核查完成时间"
                  style="width: 100%"
                />
              </el-form-item>
            </el-col>
          ...
          <el-form-item label="核查结果" prop="checkResult">
            <el-select
              ...
              style="width: 100%"
            >
              <el-option label="线索属实" value="线索属实" />
              <el-option label="部分属实" value="部分属实" />
              <el-option label="线索不实" value="线索不实" />
              <el-option label="需进一步核查" value="需进一步核查" />
            </el-select>
          </el-form-item>

          <el-form-item label="核查情况说明" prop="checkDescription">
            <el-input
              v-model="formData.checkDescription"
              type="textarea"
              :rows="4"
              placeholder="请详细描述核查的具体情况和发现的问题"
              maxlength="1000"
              show-word-limit
            />
          </el-form-item>

          <el-form-item label="发现的违法行为" prop="violationsBehavior">
            <el-input
              v-model="formData.violationsBehavior"
              type="textarea"
              :rows="3"
              placeholder="请描述发现的具体违法行为（如无违法行为可不填）"
              maxlength="800"
              show-word-limit
            />
          </el-form-item>
            ...
            <el-col :span="12">
              <el-form-item label="处理措施" prop="handlingMeasures">
                <el-select
                  ...
                  style="width: 100%"
                >
                  <el-option label="立案查处" value="立案查处" />
                  <el-option label="责令整改" value="责令整改" />
                  <el-option label="行政处罚" value="行政处罚" />
                  <el-option label="移送司法" value="移送司法" />
                  <el-option label="教育警告" value="教育警告" />
            ...
            <el-col :span="12">
              <el-form-item label="核查状态" prop="checkStatus">
                ...
                >
                  ...
                  <el-option label="暂停核查" value="暂停核查" />
                </el-select>
              </el-form-item>
            </el-col>
          </el-row>

          <el-form-item label="核查人员" prop="checkPersonnel">
            <el-input v-model="formData.checkPersonnel" placeholder="请输入核查人员姓名" />
          </el-form-item>
          ...
          <el-form-item label="核查报告">
            ...
            >
              ...
              <template #tip>
                <div class="el-upload__tip">
                  核查报告、处理决定书等，支持 PDF/Word 文件，且不超过 20MB
                </div>
              </template>
            </el-upload>
          </el-form-item>
...
// 表单验证规则
const formRules = {
  checkStartTime: [{ required: true, message: '请选择核查开始时间', trigger: 'change' }],
  checkEndTime: [{ required: true, message: '请选择核查完成时间', trigger: 'change' }],
  checkResult: [{ required: true, message: '请选择核查结果', trigger: 'change' }],
  checkDescription: [{ required: true, message: '请输入核查情况说明', trigger: 'blur' }],
  handlingMeasures: [{ required: true, message: '请选择处理措施', trigger: 'change' }],
  checkStatus: [{ required: true, message: '请选择核查状态', trigger: 'change' }],
  checkPersonnel: [{ required: true, message: '请输入核查人员', trigger: 'blur' }],
...
const completeIssue = async () => {
  ...
  try {
    await ElMessageBox.confirm('确认完成该线索核查问题的处理吗？完成后将无法修改。', '确认完成', {
